...
public class TeamsConversationBot : TeamsActivityHandler
{
    ...
    /// <param name="cancellationToken"></param>
    /// <returns>A task that represents the work queued to execute.</returns>
    private static Task ResetReadUserCount(ITurnContext<IMessageActivity> _, CancellationToken __)
    {
    ...
    protected override Task OnTeamsReadReceiptAsync(ReadReceiptInfo readReceiptInfo, ITurnContext<IEventActivity> turnContext, CancellationToken cancellationToken)
    {
        ...
        if (readReceiptInfo.IsMessageRead(messageId))
        {
            ...
            teamMemberMessageIdDetails.TryRemove(turnContext.Activity.From.AadObjectId, out string _);
        }
